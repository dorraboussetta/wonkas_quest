...
#define MAX_PLATFORMS 64        /* bumped — txt files can hold many platforms */
 ...
 * Reads a plain-text file and fills the platforms array.
 *
 * File format (one platform per line):
 *   type  x  y  w  h  vel_x  vel_y  image_path
 *
 * Lines that start with '#' or are blank are ignored.
 *
 * Returns the number of platforms loaded, or -1 on error.
 * *count is set to the same value on success.
 */
...
int  init_level(Platform *platforms, int *count,
                SDL_Renderer *r, int level);   /* kept for fallback */
